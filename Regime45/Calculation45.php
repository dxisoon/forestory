...
// generate Volume TreeNumber ,Status , Production, Cut_Angle
...
{
    // DBH in metres (diameter stored as cm); 0.15 m = 15 cm threshold for small/large allometry
    $Dia = (float) ($diam / 100);
    $Hie = (float) $stemH;

    if ($spgroup <= 4) { // Dipterocarp species groups 1–4
        if ($Dia < 0.15) {
            $vol = 0.022 + (3.4 * pow($Dia, 2));
        } else {
            $vol = 0.015 + (2.137 * pow($Dia, 2)) + (0.513 * pow($Dia, 2) * $Hie);
        }
    } else { // Non-dipterocarp groups 5–7
        if ($Dia < 0.15) {
...
{
    if (in_array($diamC, [1, 2, 3, 5]) && $diam > 45) {
        return "Cut";
    } else {
        return "Keep";
    }
...
for ($i = 0; $i < count($id); $i++) {
    $batch_updates[] = "UPDATE new_forest
        SET
            Volume = {$volume[$i]},
...
echo "Total Data Processed in Calculation45: " . count($id) . "<br>";

include 'Victim45.php';  // Corrected include path
$conn->close();
?>